    ...
    try {
      ...
      if (is3D) {
        try {
          ...
          const plotlyDiv =
            chartContainer.querySelector("div[data-plotly='true']") ||
          ...
          alert(
            "Unable to export 3D chart. Plotly export functionality not available."
          );
          return;
        } catch (error) {
      ...
      if (!canvas) {
        alert(
          "2D chart canvas not found! Make sure the chart is fully loaded."
        );
    ...
    try {
      if (format === "pdf") {
        ...
        try {
          ...
          console.warn(
            "Direct canvas export failed, trying alternative method:",
            error
          );
