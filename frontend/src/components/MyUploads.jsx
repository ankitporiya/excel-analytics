...
const MyUploads = () => {
  ...
  const { files, currentFile, loading, error } = useSelector(
    (state) => state.files
  );
  // console.log(files);
  ...
  const renderPreviewData = (data) => {
    ...
    if (Array.isArray(data.data)) {
      ...
      allRows = Object.entries(data.data).map(([key, value]) => ({
        key,
        value,
      }));
    }
    ...
    const previewIndexOfFirstItem =
      previewIndexOfLastItem - previewItemsPerPage;
    const currentPreviewRows = allRows.slice(
      previewIndexOfFirstItem,
      previewIndexOfLastItem
    );
    ...
    if (
      ...
      typeof currentPreviewRows[0] === "object" &&
      !("key" in currentPreviewRows[0])
    ) {
        ...
        <div>
          ...
          <div className="mb-4 p-4 bg-gradient-to-r from-[#f8fff8] to-[#e6f3e6] rounded-lg">
            <p className="text-sm text-[#228B22] font-medium">
              Showing {previewIndexOfFirstItem + 1}-
              {Math.min(previewIndexOfLastItem, allRows.length)} of{" "}
            ...
            <table className="min-w-full bg-white border border-gray-200 rounded-lg">
              <thead className="bg-gradient-to-r from-[#f8fff8] to-[#e6f3e6]">
                <tr>
                  {headers.map((header, index) => (
                    <th
                      key={index}
                      className="px-4 py-3 text-left text-sm font-semibold text-[#228B22] border-b border-gray-200"
                    >
                      {header}
                    </th>
                  ))}
              ...
              <tbody>
                {currentPreviewRows.map((row, index) => (
                  <tr
                    key={index}
                    className={index % 2 === 0 ? "bg-[#f8fff8]" : "bg-white"}
                  >
      ...
      <div>
        ...
        <div className="space-y-2 mb-4">
          {currentPreviewRows.map((item, index) => (
            <div
              key={index}
              className="flex items-center justify-between p-3 bg-[#f8fff8] rounded-lg border border-gray-200"
            >
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gradient-to-r from-[#f8fff8] to-[#e6f3e6]">
                      <tr>
                        <th className="px-4 md:px-6 py-4 text-left text-sm font-semibold text-[#228B22]">
                          Name
                        </th>
                        <th className="px-4 md:px-6 py-4 text-left text-sm font-semibold text-[#228B22] hidden sm:table-cell">
                          Date
                        </th>
                        <th className="px-4 md:px-6 py-4 text-left text-sm font-semibold text-[#228B22] hidden md:table-cell">
                          Size
                        </th>
                        <th className="px-4 md:px-6 py-4 text-left text-sm font-semibold text-[#228B22]">
                          Actions
                      ...
                      <AnimatePresence>
                        {currentFiles.map((file, index) => (
                          <motion.tr
                            ...
                            className={`${
                              index % 2 === 0 ? "bg-[#f8fff8]" : "bg-white"
                            } hover:bg-[#e6f3e6] transition-colors duration-200`}
                          >
                            <td className="px-4 md:px-6 py-4">
                              <div className="flex items-center">
                                ...
                                <div className="min-w-0 flex-1">
                                  <p
                                    className="text-sm font-medium text-[#228B22] truncate"
                                    title={
                                      file.originalFileName || "Unknown File"
                                    }
                                  >
              ...
              >
                ...
                >
                  <div className="min-w-0 flex-1 mr-4">
                    <h2 className="text-xl md:text-2xl font-bold text-[#228B22]">
                      File Preview
                    </h2>
                    {previewData && (
                      <p className="text-gray-600 mt-1 font-medium text-sm md:text-base truncate">
                ...
                >
                  {previewData ? (
                    ...
                    <div className="flex items-center justify-center py-12">
                      <div className="w-8 h-8 border-4 border-[#228B22] border-t-transparent rounded-full animate-spin"></div>
                      <span className="ml-3 text-[#228B22] font-medium">
                        Loading preview...
                      </span>
                    </div>
                  )}
